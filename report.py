...
def _write_summary(wb: Workbook, result: ReconciliationResult, ai_insights: str, tds_result=None, cost_tracker=None) -> None:
    ...
    stats = [
        ...
        ("L3 matches (Date + Amount, no ref)", s["matched_l3"]),
        ("Matched incl. TDS (counterparty withheld TDS)", s.get("matched_tds", 0)),
        ("Amount mismatches", s["amount_mismatches"]),
        ("TDS - needs verify (both sides carry TDS)", s.get("suggested_tds_unverified", 0)),
    ...
    walk = [
        ...
        ("  of which: TDS withheld, not yet booked by counterparty", s.get("tds_match_delta", 0)),
        ("Reconciling items (missing + mismatches + TDS)", s["reconciling_item"]),
